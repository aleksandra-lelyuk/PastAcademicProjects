/*
 * AVL tree implementation adapted from https://www.geeksforgeeks.org/avl-tree-set-2-deletion/
 */
...
public class TreeAVL<T> {
	...
	public T remove(int k){
		Node<T> found = deleteNode(root, k);
		if (found != null)
			return found.value;
		else
			return null;
	...
	private Node<T> search(Node<T> root, int k){
		if(root != null) {
			if (k < root.key) {
	...
	private Node<T> insert(Node node, int key, T value) {
		/* 1. Perform the normal BST rotation */
		if (node == null) {
			System.out.println("Inserted new object");
	...
	private Node<T> deleteNode(Node<T> root, int key) {
